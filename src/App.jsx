...
const STAT_WIDGETS = [
  ...
  { id:"cost_per_km",       label:"Náklady na palivo / 1 km",       icon:"🚗" },
  { id:"repairs_monthly",   label:"Náklady na opravy / měsíc",      icon:"🔧" },
  { id:"vehicles_compare",  label:"Srovnání vozidel",               icon:"🚘" },
];
...
const StatsMod = ({ fueling, repairs, addons, vehicles, activeVid, user }) => {
  ...
  // Filtrování
  const filterDate = (arr) => arr.filter(f => {
    if(fFrom && f.date < fFrom) return false;
    if(fTo && f.date > fTo) return false;
    return true;
  });

  const vf = filterDate(fueling.filter(f => f.vid === activeVid));
  const vr = filterDate(repairs.filter(r => r.vid === activeVid));
  const va = filterDate(addons.filter(a => a.vid === activeVid));
  ...
  const monthlyCost = useMemo(()=>{ const map={}; vf.forEach(f=>{ const k=monthKey(f.date); map[k]=(map[k]||0)+(f.total||0); }); return Object.keys(map).sort().map(k=>({ m:monthLabel(k), v:Math.round(map[k]) })); },[vf]);
  const avgPrice = useMemo(()=>{ const map={}; vf.forEach(f=>{ const k=monthKey(f.date); if(!map[k]) map[k]=[]; map[k].push(f.pricePerLiter||0); }); return Object.keys(map).sort().map(k=>({ m:monthLabel(k), v:Math.round(map[k].reduce((a,b)=>a+b,0)/map[k].length*100)/100 })); },[vf]);
  const consumptionData = useMemo(()=>{ const sorted=[...vf].sort((a,b)=>new Date(a.date)-new Date(b.date)||a.km-b.km); return sorted.map((f,i)=>{ const prev=sorted[i-1]; const driven=prev?f.km-prev.km:null; const cons=driven&&driven>0?(f.liters/driven*100):null; return cons?{ m:monthLabel(monthKey(f.date)), v:+cons.toFixed(2) }:null; }).filter(Boolean); },[vf]);
  const breakdown = useMemo(()=>{ const fuel=vf.reduce((s,f)=>s+(f.total||0),0); const rep=vr.reduce((s,r)=>s+(r.matPrice||0)+(r.laborPrice||0),0); const add=va.reduce((s,a)=>s+(a.price||0),0); return [{ name:"Palivo", value:Math.round(fuel) },{ name:"Opravy", value:Math.round(rep) },{ name:"Doplňky", value:Math.round(add) }].filter(x=>x.value>0); },[vf,vr,va]);
  const yearlyTotal = useMemo(()=>{ const map={}; [...fueling.filter(f=>f.vid===activeVid).map(f=>({y:new Date(f.date).getFullYear(),v:f.total||0})),...repairs.filter(r=>r.vid===activeVid).map(r=>({y:new Date(r.date).getFullYear(),v:(r.matPrice||0)+(r.laborPrice||0)})),...addons.filter(a=>a.vid===activeVid).map(a=>({y:new Date(a.date).getFullYear(),v:a.price||0}))].forEach(({y,v})=>{ map[y]=(map[y]||0)+v; }); return Object.keys(map).sort().map(k=>({ m:k, v:Math.round(map[k]) })); },[fueling,repairs,addons,activeVid]);
  const stationStats = useMemo(()=>{ const map={}; vf.forEach(f=>{ if(!f.location) return; const k=f.location.split(" ")[0]||f.location; map[k]=(map[k]||0)+1; }); return Object.entries(map).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([k,v])=>({ m:k, v })); },[vf]);
  const costPerKm = useMemo(()=>{ const sorted=[...vf].sort((a,b)=>new Date(a.date)-new Date(b.date)||a.km-b.km); const map={}; sorted.forEach((f,i)=>{ const prev=sorted[i-1]; if(!prev) return; const km=f.km-prev.km; if(km<=0) return; const k=monthKey(f.date); if(!map[k]) map[k]={cost:0,km:0}; map[k].cost+=(f.total||0); map[k].km+=km; }); return Object.keys(map).sort().map(k=>({ m:monthLabel(k), v:Math.round(map[k].cost/map[k].km*100)/100 })); },[vf]);
  const repairsMonthly = useMemo(()=>{ const map={}; vr.forEach(r=>{ const k=monthKey(r.date); map[k]=(map[k]||0)+(r.matPrice||0)+(r.laborPrice||0); }); return Object.keys(map).sort().map(k=>({ m:monthLabel(k), v:Math.round(map[k]) })); },[vr]);

  // Srovnání vozidel - celkové náklady
  const vehiclesCompare = useMemo(()=>{
    return vehicles.map(v => {
      const vFuel = filterDate(fueling.filter(f=>f.vid===v.id));
      const vRep  = filterDate(repairs.filter(r=>r.vid===v.id));
      const vAdd  = filterDate(addons.filter(a=>a.vid===v.id));
      const fuel  = vFuel.reduce((s,f)=>s+(f.total||0),0);
      const rep   = vRep.reduce((s,r)=>s+(r.matPrice||0)+(r.laborPrice||0),0);
      const add   = vAdd.reduce((s,a)=>s+(a.price||0),0);
      return { name: v.brand+" "+v.model, fuel:Math.round(fuel), rep:Math.round(rep), add:Math.round(add), total:Math.round(fuel+rep+add) };
    }).filter(v=>v.total>0);
  },[vehicles,fueling,repairs,addons,fFrom,fTo]);
  ...
  const fmtAxis = v => v>=10000 ? Math.round(v/1000)+"k" : v>=1000 ? (v/1000).toFixed(1)+"k" : String(v);
  ...
  const renderWidget = (id) => {
    ...
    const titleBar = (t, data, unit="") => {
    const vals = data.map(d=>d.v).filter(v=>typeof v==="number"&&!isNaN(v));
    const avg = vals.length ? Math.round(vals.reduce((a,b)=>a+b,0)/vals.length*100)/100 : null;
    return (
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{fontSize:12,fontWeight:600,color:"var(--t2)",letterSpacing:".06em",textTransform:"uppercase"}}>{t}</div>
        {avg!==null&&<div style={{fontSize:11,color:"var(--acc)",background:"rgba(200,169,110,.12)",borderRadius:6,padding:"3px 8px",fontWeight:500}}>⌀ {fmt(avg,2)}{unit&&" "+unit}</div>}
      </div>
    );
  };
  const title = (t) => <div style={{fontSize:12,fontWeight:600,color:"var(--t2)",letterSpacing:".06em",textTransform:"uppercase",marginBottom:12}}>{t}</div>;
    ...
    switch(id) {
      case "monthly_cost": return <div key={id} style={wStyle}>{titleBar("💰 Náklady na palivo / měsíc", monthlyCost, "Kč")}{monthlyCost.length<2?noData:<ResponsiveContainer width="100%" height={220}><BarChart data={monthlyCost} margin={{top:0,right:0,left:-20,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="var(--b1)"/><XAxis dataKey="m" tick={{fontSize:10,fill:"var(--t3)"}} interval={xInterval(monthlyCost)}/><YAxis tick={{fontSize:10,fill:"var(--t3)"}} domain={["auto","auto"]} width={55} tickFormatter={fmtAxis} label={{value:"Kč",angle:-90,position:"insideLeft",offset:14,style:{fontSize:10,fill:"var(--t3)"}}}/><Tooltip contentStyle={TT_STYLE} formatter={v=>fmt(v)+" Kč"}/><Bar dataKey="v" fill="var(--acc)" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer>}</div>;
      case "avg_price": return <div key={id} style={wStyle}>{titleBar("📈 Průměrná cena paliva v čase", avgPrice, "Kč/L")}{avgPrice.length<2?noData:<ResponsiveContainer width="100%" height={220}><AreaChart data={avgPrice} margin={{top:0,right:0,left:-20,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="var(--b1)"/><XAxis dataKey="m" tick={{fontSize:10,fill:"var(--t3)"}} interval={xInterval(avgPrice)}/><YAxis tick={{fontSize:10,fill:"var(--t3)"}} domain={["auto","auto"]} width={55} tickFormatter={fmtAxis} label={{value:"Kč/L",angle:-90,position:"insideLeft",offset:14,style:{fontSize:10,fill:"var(--t3)"}}}/><Tooltip contentStyle={TT_STYLE} formatter={v=>fmt(v,2)+" Kč/L"}/><Area type="monotone" dataKey="v" stroke="var(--green)" fill="rgba(78,203,113,.15)" strokeWidth={2}/></AreaChart></ResponsiveContainer>}</div>;
      case "consumption": return <div key={id} style={wStyle}>{titleBar("⛽ Spotřeba v čase", consumptionData, "L/100km")}{consumptionData.length<2?noData:<ResponsiveContainer width="100%" height={220}><AreaChart data={consumptionData} margin={{top:0,right:0,left:-20,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="var(--b1)"/><XAxis dataKey="m" tick={{fontSize:10,fill:"var(--t3)"}} interval={xInterval(consumptionData)}/><YAxis tick={{fontSize:10,fill:"var(--t3)"}} domain={["auto","auto"]} width={55} tickFormatter={fmtAxis} label={{value:"L/100",angle:-90,position:"insideLeft",offset:14,style:{fontSize:10,fill:"var(--t3)"}}}/><Tooltip contentStyle={TT_STYLE} formatter={v=>fmt(v,2)+" L/100km"}/><Area type="monotone" dataKey="v" stroke="var(--acc)" fill="rgba(200,169,110,.15)" strokeWidth={2}/></AreaChart></ResponsiveContainer>}</div>;
      case "cost_breakdown": return <div key={id} style={wStyle}>{title("🥧 Rozložení nákladů")}{breakdown.length===0?noData:<ResponsiveContainer width="100%" height={200}><PieChart><Pie data={breakdown} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({name,percent})=>name+" "+Math.round(percent*100)+"%"} labelLine={false}>{breakdown.map((e,i)=><Cell key={i} fill={PIE_COLORS[i%PIE_COLORS.length]}/>)}</Pie><Tooltip contentStyle={TT_STYLE} formatter={v=>fmt(v)+" Kč"}/></PieChart></ResponsiveContainer>}</div>;
      case "total_by_year": return <div key={id} style={wStyle}>{titleBar("📅 Roční přehled celkových nákladů", yearlyTotal, "Kč")}{yearlyTotal.length===0?noData:<ResponsiveContainer width="100%" height={220}><BarChart data={yearlyTotal} margin={{top:0,right:0,left:-20,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="var(--b1)"/><XAxis dataKey="m" tick={{fontSize:10,fill:"var(--t3)"}}/><YAxis tick={{fontSize:10,fill:"var(--t3)"}} domain={["auto","auto"]} width={55} tickFormatter={fmtAxis} label={{value:"Kč",angle:-90,position:"insideLeft",offset:14,style:{fontSize:10,fill:"var(--t3)"}}}/><Tooltip contentStyle={TT_STYLE} formatter={v=>fmt(v)+" Kč"}/><Bar dataKey="v" fill="var(--blue)" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer>}</div>;
      case "station_stats": return <div key={id} style={wStyle}>{title("📍 Oblíbené čerpací stanice")}{stationStats.length===0?noData:<ResponsiveContainer width="100%" height={220}><BarChart data={stationStats} layout="vertical" margin={{top:0,right:10,left:30,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="var(--b1)"/><XAxis type="number" tick={{fontSize:10,fill:"var(--t3)"}}/><YAxis type="category" dataKey="m" tick={{fontSize:11,fill:"var(--t2)"}} width={60}/><Tooltip contentStyle={TT_STYLE} formatter={v=>v+" x"}/><Bar dataKey="v" fill="var(--acc2)" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer>}</div>;
      case "cost_per_km": return <div key={id} style={wStyle}>{titleBar("🚗 Náklady na palivo / 1 km", costPerKm, "Kč/km")}{costPerKm.length<2?noData:<ResponsiveContainer width="100%" height={220}><AreaChart data={costPerKm} margin={{top:0,right:0,left:-20,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="var(--b1)"/><XAxis dataKey="m" tick={{fontSize:10,fill:"var(--t3)"}} interval={xInterval(costPerKm)}/><YAxis tick={{fontSize:10,fill:"var(--t3)"}} domain={["auto","auto"]} width={55} tickFormatter={fmtAxis} label={{value:"Kč/km",angle:-90,position:"insideLeft",offset:14,style:{fontSize:10,fill:"var(--t3)"}}}/><Tooltip contentStyle={TT_STYLE} formatter={v=>fmt(v,2)+" Kč/km"}/><Area type="monotone" dataKey="v" stroke="var(--red)" fill="rgba(224,92,92,.15)" strokeWidth={2}/></AreaChart></ResponsiveContainer>}</div>;
      case "repairs_monthly": return <div key={id} style={wStyle}>{titleBar("🔧 Náklady na opravy / měsíc", repairsMonthly, "Kč")}{repairsMonthly.length===0?noData:<ResponsiveContainer width="100%" height={220}><BarChart data={repairsMonthly} margin={{top:0,right:0,left:-20,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="var(--b1)"/><XAxis dataKey="m" tick={{fontSize:10,fill:"var(--t3)"}} interval={xInterval(repairsMonthly)}/><YAxis tick={{fontSize:10,fill:"var(--t3)"}} domain={["auto","auto"]} width={55} tickFormatter={fmtAxis} label={{value:"Kč",angle:-90,position:"insideLeft",offset:14,style:{fontSize:10,fill:"var(--t3)"}}}/><Tooltip contentStyle={TT_STYLE} formatter={v=>fmt(v)+" Kč"}/><Bar dataKey="v" fill="var(--red)" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer>}</div>;
      case "vehicles_compare": return (
        <div key={id} style={wStyle}>
          {title("🚘 Srovnání vozidel – celkové náklady")}
          {vehiclesCompare.length===0 ? noData :
          <>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={vehiclesCompare} margin={{top:0,right:0,left:-20,bottom:0}}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--b1)"/>
                <XAxis dataKey="name" tick={{fontSize:10,fill:"var(--t3)"}}/>
                <YAxis tick={{fontSize:10,fill:"var(--t3)"}} domain={["auto","auto"]} width={45} tickFormatter={fmtAxis}/>
                <Tooltip contentStyle={TT_STYLE} formatter={v=>fmt(v)+" Kč"}/>
                <Legend wrapperStyle={{fontSize:11,color:"var(--t2)"}}/>
                <Bar dataKey="fuel" name="Palivo" stackId="a" fill="var(--acc)" radius={[0,0,0,0]}/>
                <Bar dataKey="rep" name="Opravy" stackId="a" fill="var(--blue)"/>
                <Bar dataKey="add" name="Doplňky" stackId="a" fill="var(--green)" radius={[4,4,0,0]}/>
              </BarChart>
            </ResponsiveContainer>
            <div style={{display:"flex",gap:8,marginTop:10,flexWrap:"wrap"}}>
              {vehiclesCompare.map(v=>(
                <div key={v.name} style={{flex:1,minWidth:120,background:"var(--s2)",borderRadius:10,padding:"10px 12px"}}>
                  <div style={{fontSize:11,color:"var(--t3)",marginBottom:4}}>{v.name}</div>
                  <div style={{fontSize:15,fontWeight:600,color:"var(--acc)"}}>{fmt(v.total)} Kč</div>
                </div>
              ))}
            </div>
          </>}
        ...
        <div style={{background:"var(--s2)",border:"1px solid var(--b1)",borderRadius:14,padding:16,marginBottom:16}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {STAT_WIDGETS.map(w=>{
              ...
              return (
                <div key={w.id} onClick={()=>toggle(w.id)} style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer",padding:"10px 12px",borderRadius:10,background:on?"rgba(200,169,110,.08)":"var(--s1)",border:"1px solid",borderColor:on?"var(--acc)":"var(--b1)"}}>
                  <div style={{width:20,height:20,borderRadius:5,border:"2px solid",borderColor:on?"var(--acc)":"var(--b2)",background:on?"var(--acc)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all .15s"}}>
                    {on&&<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><polyline points="1.5,6 5,9.5 10.5,2.5" stroke="#0a0a0a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                  </div>
                  <span style={{fontSize:14,color:on?"var(--t1)":"var(--t2)"}}>{w.icon} {w.label}</span>
                </div>
              );
            })}
          </div>
...
const VForm = ({existing,onSave,onClose}) => {
  ...
  s("spz", fmt);
}} placeholder="1AB 2345" maxLength={9}/></FR>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:24,backdropFilter:"blur(10px)"}} onClick={()=>setShowExport(false)}>
          <div style={{background:"var(--s1)",border:"1px solid var(--b2)",borderRadius:20,maxWidth:360,width:"100%",maxHeight:"85dvh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div style={{fontSize:10,fontWeight:500,letterSpacing:".12em",color:"var(--t3)",textTransform:"uppercase",margin:"16px 0 4px"}}>Import CSV</div>
              <div style={{fontSize:12,color:"var(--t3)",marginBottom:8,lineHeight:1.5}}>Importuj historická data z CSV souboru. Sloupce: datum;misto;palivo;litry;cena_za_litr;celkem_kc;km</div>
              <label style={{background:"var(--s2)",border:"1px solid var(--b1)",borderRadius:10,padding:"12px 16px",color:"var(--t1)",fontSize:14,textAlign:"left",touchAction:"manipulation",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
                <span style={{fontSize:20}}>📥</span> Importovat tankování (.csv)
                <input type="file" accept=".csv" style={{display:"none"}} onChange={async e=>{
                  ...
                  for(let i=1;i<lines.length;i++){
                    ...
                    const rec = {
                      vid:activeVid,
                      date,
                      location:row["misto"]||row["location"]||"",
                      fuelType:row["palivo"]||row["fuel_type"]||row["fueltype"]||"Natural 95",
                      liters,
                      pricePerLiter:ppl||Math.round(total/liters*100)/100||0,
                      total:total||Math.round(liters*ppl*100)/100||0,
                      km
                    };
                    await saveFuel(rec, null);
                    imported++;
                  }
