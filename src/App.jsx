...
const StatsMod = ({ fueling, repairs, addons, vehicles, activeVid, user }) => {
  ...
  const title = (t) => <div style={{fontSize:12,fontWeight:600,color:"var(--t2)",letterSpacing:".06em",textTransform:"uppercase",marginBottom:12}}>{t}</div>;
    ...
    switch(id) {
      ...
      case "total_by_year": return <div key={id} style={wStyle}>{titleBar("📅 Roční přehled celkových nákladů", yearlyTotal, "Kč")}{yearlyTotal.length===0?noData:<ResponsiveContainer width="100%" height={220}><BarChart data={yearlyTotal} margin={{top:0,right:0,left:-20,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke="var(--b1)"/><XAxis dataKey="m" tick={{fontSize:10,fill:"var(--t3)"}}/><YAxis tick={{fontSize:10,fill:"var(--t3)"}} domain={["auto","auto"]} width={48} label={{value:"Kč",angle:-90,position:"insideLeft",offset:12,style:{fontSize:10,fill:"var(--t3)"}}}/><Tooltip contentStyle={TT_STYLE} formatter={v=>fmt(v)+" Kč"}/><Bar dataKey="v" fill="var(--blue)" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer>}</div>;
          ...
          <>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={vehiclesCompare} margin={{top:0,right:0,left:-20,bottom:0}}>
                ...
                <YAxis tick={{fontSize:10,fill:"var(--t3)"}} domain={["auto","auto"]} width={40}/>
